...
export default function Newton_dividedPage() {
	...
	const { error, data, isFetched, isError, isFetching, refetch } = useQuery({
		queryKey: ["newton_dvided"],
		queryFn: () =>
			fetch_server({
				endpoint: "/interpolation/newton_divided",
				data: {
			...
			>
				...
				<div className="flex flex-col justify-center w-full items-center mb-6 rounded-md p-6 bg-white shadow-md">
					...
					<input
						...
						className="p-3 mb-4 mt-4  w-20 text-center spin h-10 rounded-md border-[1px] border-gray-300"
						type="number"
						name="size"
						id="matrix-size"
						min={2}
