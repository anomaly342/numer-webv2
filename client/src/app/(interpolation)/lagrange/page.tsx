...
import LagrangeSolution from "@/components/resultTable/LagrangeSolution";
...
export default function LagrangePage() {
	...
	const { error, data, isFetched, isError, isFetching, refetch } = useQuery({
		...
		queryFn: () =>
			fetch_server({
				endpoint: "/interpolation/lagrange",
				data: {
		...
		<main className="w-full flex flex-col py-6 items-center px-4">
			...
			<div className="flex flex-col items-center max-w-7xl w-full mt-12">
				...
				{isFetched &&
					!isFetching &&
					(!isError ? (
						<LagrangeSolution data={data as LagrangeResult}></LagrangeSolution>
					) : (
						error.message
					))}
			</div>
